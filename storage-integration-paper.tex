...
\label{s-rw}
To the best of our knowledge, alternative solutions for integrating external storage systems with a blockchain network does not exist. So we describe the few blockchain based storage solutions that are published in recent years for the sake of completion.   

Swarm: DHT (content addressed chunk store), incentive for serving retrieval request and option for long term commitment whose violation confiscate miner's deposit. The terms and conditions for storage requirement and payment for retrieval is typically handled with blockchain smart contracts. Users ensure persistence by storing replicas and a chunk content hash. No provision for confidentiality from other  users or protection from miner's eavesdropping on document. Owner gets receipts for promises. Contract negotiation done using escrow. A node can request other nodes to store chunks also.



   

Filecoint, swarm, storj, ipfs
